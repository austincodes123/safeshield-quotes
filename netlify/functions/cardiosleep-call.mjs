// Emails Austin when the Dr. Surkin books a call from quotes.gosafeshield.com/cardiosleep/.
...
<html><body style="margin:0;padding:28px 16px;background:#eef1f5;
  ...
  <table role="presentation" cellpadding="0" cellspacing="0" border="0"
    ...
    <tr>
      <td style="padding:26px 30px 20px;border-bottom:1px solid #e6e9ee;">
        <div style="display:inline-block;background:#12294a;color:#ffffff;font-size:10px;letter-spacing:.14em;
                    text-transform:uppercase;padding:5px 12px;border-radius:99px;">Call booked</div>
        <div style="font-size:23px;font-weight:600;color:#12294a;margin-top:14px;">
          ${esc(d.client || 'CardioSleep Diagnostics')} booked a call
        </div>
      </td>
    </tr>
    ...
    <tr>
      <td style="padding:22px 30px 6px;">
        <div style="background:#f6f8fa;border-left:3px solid #f0a63c;border-radius:0 10px 10px 0;
          ...
          <b style="font-size:16px;color:#12294a;">Put it on the calendar and send the Meet invite.</b>
          <span style="display:block;font-size:13px;color:#6b7a8f;margin-top:5px;">
            They were told a calendar invite is on its way.
          </span>
        </div>
      </td>
    </tr>
    ...
    <tr>
      <td style="padding:18px 30px 4px;">
        <div style="font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:#12294a;
                    font-weight:700;padding-bottom:6px;">The call</div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
          ${row('Day', d.call_date, { strong: true, big: true })}
          ${row('Time', d.call_time, { strong: true, big: true })}
          ${row('Best number', d.phone)}
        </table>
      </td>
    </tr>
    ...
    <tr>
      <td style="padding:18px 30px 4px;">
        <div style="font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:#12294a;
                    font-weight:700;padding-bottom:6px;">Where they landed on the page</div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
          ${row('Cyber protection', d.cyber, { strong: true })}
          ${row('Billing plan viewed', d.billing_plan)}
          ${row('Monthly shown', d.monthly_total, { strong: true })}
        </table>
      </td>
    </tr>
...
export default async (req) => {
  ...
  const subject = `${d.client || 'CardioSleep'} booked a call: ${d.call_date || ''} at ${d.call_time || ''}`;
